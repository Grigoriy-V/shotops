...
69 GiB of weights is a fixed cost that dwarfs the sampling either side of it.
The jump to thirty is a different story — three times the wall clock — and the
...
content filter between an idea and a result. Paid for with a GPU to keep fed.
Neither is the winner; the point is that the scene spec did not notice.
